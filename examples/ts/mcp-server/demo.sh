...
# MCP Server Demo Suite (TypeScript)
...
start_section "🤖 MCP Server — TypeScript" "Telnyx MCP server exposing API tools to AI agents via streamable HTTP"
...
finish_tests_rich "The TypeScript MCP server demonstrates how to run a Model Context Protocol server as a Telnyx Edge Function. The telnyx-mcp library handles the MCP protocol, tool registration, and code execution — this example just wires it into an Express HTTP server with Knative health probes. AI agents connect via streamable HTTP transport on POST / and can use two tools: 'execute' to run TypeScript code against the Telnyx SDK, and 'search_docs' to find SDK documentation."
